...
$collegeInsertData = json_decode($requestBody,true);
...
function validDataInputs($values){
 ...
 function find($id, $connection){
    $search = 'SELECT collid FROM colleges WHERE collid = ?;';
 ...
 if(validDataInputs($collegeInsertData)){
 
     // echo 'All inputs are valid.';
     if(find($collegeInsertData['collid'], $dbconnection) > 0){
         $message['status'] = 'College ID already exists in the system.';
         $message['code'] = 300;
     } else {
         ...
         $dbStatment->bindParam(1,$collegeInsertData['collid'],PDO::PARAM_INT);
         $dbStatment->bindParam(2,$collegeInsertData['collfullname'],PDO::PARAM_STR);
         $dbStatment->bindParam(3,$collegeInsertData['collshortname'],PDO::PARAM_STR);
